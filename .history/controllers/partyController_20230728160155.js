...
const partyController = {
  ...
  delete: async (req, res) => {
    try {
      ...
      const parties = await PartyModel.findById(id)
      if (!parties) {
        res.status(404).json({ msg: 'Festa não encontrada ou já deletada do banco de dados.' })
        return
      }
      const deleteParty = await PartyModel.findByIdAndDelete(id)
      res.status(200).json({ deleteParty, msg: 'Festa exclída com sucesso!' })
    } catch (error) {
  ...
  update: async (req, res) => {
    try {
      ...
      res.status(200).json({ service, msg: 'Festa atualizado com sucesso!' })
    } catch (error) {
      console.log('Erro ao atualizar dado específico: ', error)
    }
  }
}
